...
chmod +x ./mbq/v0.83/miniBloq-amd64
chmod +x ./mbq/v0.83/miniBloq-i386
...
ln -sf `which python` ./lang/PPythonLinux/python
...
if [ "${OS}" = "Linux" ] ; then
    ...
    echo $MACH
    if [ "${MACH}" = "x86_64" ] ; then
        export LD_LIBRARY_PATH=$PWD/mbq/v0.83/linuxRuntime/amd64/
        export PYTHONPATH=$PWD/mbq/v0.83/../../lang/PPythonLinux/site-packages
        $PWD/mbq/v0.83/miniBloq-amd64
    else
#    if [ "${MACH}" = "i386" ] || [ "${MACH}" = "i486" ] || [ "${MACH}" = "i586" ] || [ "${MACH}" = "i686" ] ; then
        export LD_LIBRARY_PATH=$PWD/mbq/v0.83/linuxRuntime/i386/
        export PYTHONPATH=$PWD/mbq/v0.83/../../lang/PPythonLinux/site-packages
        $PWD/mbq/v0.83/miniBloq-i386
    fi
else
