...
//IRRemoteSF library is a modified IRLibrary whcih supports the SparkFun's IR remote controller (https://www.sparkfun.com/products/11759):
#include "IRremoteSF.h"
#include "IRremoteIntSF.h"
...
long IRrecvSF::decodeNEC(decode_resultsSF *results) {
  long data = 0;
...
int IRrecvSF::getRClevel(decode_resultsSF *results, int *offset, int *used, int t1) {
...
long IRrecvSF::decodeJVC(decode_resultsSF *results) {
    long data = 0;
    int offset = 1; // Skip first space
    // Check for repeat
...
int IRrecvSF::compare(unsigned int oldval, unsigned int newval) {
  if (newval < oldval * .8) {
    return 0;
  } 
 ...
 */
long IRrecvSF::decodeHash(decode_resultsSF *results) {
